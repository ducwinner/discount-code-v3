import { ExtendedProduct, CpRule, PlRule, RuleInfo, PricingApplied } from '@/types';
...
function checkExcludedCustomer(rule: RuleInfo): boolean {
...
function checkAppliedCustomer(rule: RuleInfo): boolean {
...
function checkAppliedDate(rule: RuleInfo) {
    let result = true;
...
function checkAppliedCpRule(rule: CpRule, checkUnique: Map<number, PricingApplied>, isCartItem?: boolean) {
    ...
    function setAppliedRule(productId: number, product: ExtendedProduct) {
